...
总的来说，C++ 对象模型构建了从抽象语义到物理内存的映射框架。它规定了对象的布局方式、函数的调用语义、动态多态的查表机制以及生命周期的语言保障。这一机制不仅解释了 C++ 的面向对象行为，也为 RAII 等高级内存管理思想奠定了基础。
...
对于如上`Point`类生成的对象，编译器会将`x, y`分别顺序放入连续的内存空间。此外，为了提高访问性能，编译器还会按照平台对齐规则（如 4/8 字节对齐）自动在成员之间插入 padding。这导致类的实际大小往往大于其所生成的对象大小总和。
...
正如前文所言，当类第一次声明虚函数时，编译器会为该类自动生成一张虚函数表`vtable`，并在每个对象中插入一个隐藏指针`vptr`指向该表。每个派生类在继承时，会复制或重建其基类的`vtable`，并对被重写的函数项进行替换。`vptr`的初始化通常发生在构造函数的第一步。
...
C++对象模型中的多态指的是运行时多态，这也是对象模型中最具代表性的机制。它让程序能够在运行时依据对象的实际类型决定调用的函数版本。支持这种运行时多态的基本语法是虚函数。当类中存在虚函数时，编译器会自动为类生成：
...
在上面的实例中，当通过 `Base*` 调用 `speak()` 时，运行时通过 `vptr` 查表找到 `Derived::speak()`。如果函数不是虚函数，则在编译阶段已静态确定为`Base::speak()`。
...
为了解决这种棱形继承导致的重复继承问题，C++通过**虚继承**(virtual inheritance)来解决此问题，使得多条派生路径共同同一个基类子对象。
...
构造时，从基类到派生类，顺序构造；析构时，从派生类到基类，逆序销毁；对象作用域结束时自动调用析构，这就是RAII的基础逻辑。在 C++ 中，对象的创建与销毁不仅是内存事件，更是语义事件。当作用域结束时，对象会自动调用析构函数，这一行为由语言标准严格规定，不依赖运行时垃圾回收。
...
| 对齐填充 (padding) | 为保证访问效率        | 可能导致对象实际大小大于成员之和 |
| 构造/析构顺序        | 从基类到派生类 / 逆序销毁 | 生命周期受语言保障        |
